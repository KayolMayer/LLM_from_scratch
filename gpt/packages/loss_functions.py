...
def calc_loss_batch_spam(input_batch, target_batch, model, device):
    """
    Compute the cross-entropy loss for a single batch in a spam class. task.

    This function moves the input and target tensors to the specified device,
    passes the input through the model to obtain logits, and calculates the
    cross-entropy loss between the model predicted logits and the true labels.

    Parameters
    ----------
        input_batch (torch.Tensor): A batch of input tokenized sequences of
                                    shape (batch_size, seq_len).
        target_batch (torch.Tensor): A batch of target labels of shape
                                    (batch_size,).
        model (torch.nn.Module): The spam classification model.
...
def calc_loss_loader_spam(data_loader, model, device, num_batches=None):
    """
    Compute the average cross-entropy loss over multiple batches from a loader.
